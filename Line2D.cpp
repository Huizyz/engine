...
img::EasyImage draw2DLines(Lines2D& lines, const int size, Color &color) {
    // nog backGroundColor, linesColor
    ...
    // calculate the size of the image ( DIT IS OOK JUIST )
    ...
    for (Line2D& line : copy_line) {
        //hier probleem? komt niet overeen met void img::EasyImage::draw_line(unsigned int, unsigned int, unsigned int, unsigned int, const img::Color&)'
